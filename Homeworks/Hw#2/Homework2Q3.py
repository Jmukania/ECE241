...
class QuestionThree(object):
    def highest_temp(self, alist):
        ...
        """
        max = alist[0]      # Initialize max as first term
        tempIndex = 0       # Initialize max index as first index

        for i in range(1,len(alist)):  # For loop going through array checking if next term is bigger than current
            if alist[i] > max:
                max = alist[i]
                tempIndex = i
    ...
    def lowest_temp(self, alist):
        ...
        """
        min = alist[0]  # Initialize min as first term
        tempIndex = 0  # Initialize min index as first index

        for i in range(1, len(alist)):  # For loop going through array checking if next term is smaller than current

            if alist[i] < min:
                min = alist[i]
                tempIndex = i

        return min, tempIndex
    ...
    def average_high(self, alist):
        ...
        for i in range(0,len(alist)):
            sum += alist[i]             # Sum all values
            count += 1                  # Count how many values summing
