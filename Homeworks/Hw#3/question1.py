...
class HashTable:
    ...
    def put(self, key, data):
        hashvalue = self.hashfunction(key, len(self.slots))     # Finds hashvalue for slots

        if self.slots[hashvalue] == None:   # Checks status of slot
            listA = OrderedList()              # If empty... creates new ordered list
            listA.add(key, data)             # Adds key and data to ordered list
            self.slots[hashvalue] = listA      # Points hashvalue to ordered list
        else:
            # If something already in there... Means either replace or append
            listB = self.slots[hashvalue]      # Gets all objects from that hash value

            if  listB.search(key) == None:       # Checks if key already there...
                listB.add(key,data)
                self.slots[hashvalue] = listB   # Appends if not found
            else:
                listB.search(key).setData(data)  # Replaces data at specific key
                self.slots[hashvalue] = listB
