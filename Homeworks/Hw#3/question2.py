...
class BinarySearchTree:
    ...
    def _find_path(self, element_1, element_2, nature):
        ...
        list_path = []          # Path with keys
        parentRoot = self.root  # Start at root of tree
        subTreeFound = False     # Boolean to find which subtree for quickest path

        # First need to see which side of the tree to start at...
        currentRoot = parentRoot        # Tracker to find subtree for shortest path

        while not subTreeFound:
             if (currentRoot.key > element_1.key and currentRoot.key > element_2.key) or \
                     (currentRoot.key < element_1.key and currentRoot.key < element_2.key):
                # Only gets in here if route available to be shortened (either right or left)
                if currentRoot.key > element_1.key and currentRoot.key > element_2.key:
                    currentRoot = currentRoot.leftChild           # Now makes it left child and continue checking
                else:
                    currentRoot = currentRoot.rightChild          # Else make it right child and continue checking

             else:
                 subTreeFound = True    # If neither of the top is the case then must've found best sub tree

        bestRoot = currentRoot                  # Best sub tree for shortest path available

        while bestRoot.key != element_1.key:    # While loop going from best sub tree root until element 1
            list_path.insert(0,bestRoot.key)   # Insert keys at 0th index until path reached
            if element_1.key < bestRoot.key:      # Checks which way to go next
                bestRoot = bestRoot.leftChild
            else:
                bestRoot = bestRoot.rightChild

        list_path.insert(0,element_1.key)             # Need to insert last key when reached element 1
        list_path.pop(len(list_path)-1)               # Need to remove duplicate parent on the way to element 2

        # Do the same loop for element two
        bestRoot = currentRoot

        while bestRoot.key != element_2.key:  # While loop going from best root until element 2
            list_path.append(bestRoot.key)  # Appends list of path from previous list
            if element_2.key < bestRoot.key:  # Checks which way to go next
                bestRoot = bestRoot.leftChild
            else:
                bestRoot = bestRoot.rightChild

        list_path.append(element_2.key)      # Need to append last key when reached element 2
        # Number of keys is list of path

        return [list_path, len(list_path)] # TODO: uncomment this line after filling this function
